...
const __cdf = (typeof window !== 'undefined') ? '' : (await import('url')).fileURLToPath(import.meta.url);
const __cdn  =  (typeof window !== 'undefined') ? '' : __cdf.substring(0,__cdf.lastIndexOf('/')+1);
...
class Accumulator {
	...
	test(s){
		if(s.length > 1) throw new Error(`Recieved ${s.length} chars, expected 1 char`);
		if(this.c+s == this.t){
